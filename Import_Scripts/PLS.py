### This code was adapted from the bootstrapPLS package found at https://github.com/jamesacodgers/bootstrapPLS
...
class PLS:
    ...
    def boot_pdf(self,x,y):
        ...
        for i in range(self.n_b):
            ...
            p += 1/self.n_b*norm(self.boot_predict(x,i),np.sqrt(np.diag(self.boot_err_cov[i,:,:]))).pdf(y)
        return p
    ...
    def prediction_cdf(self,x,y):
        ...
        for i in range(self.n_b):
            ...
            p += 1/self.n_b*norm(self.boot_predict(x,i),np.sqrt(np.diag(self.boot_err_cov[i,:,:]))).cdf(y)
            # print(self.boot_predict(x,i))
            # print(np.diag(self.boot_err_cov[i,:,:]))
        return p
