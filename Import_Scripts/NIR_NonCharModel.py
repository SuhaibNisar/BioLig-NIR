...
training_set = pd.read_excel(file_name, sheet_name="Names",header=0).loc[:,'Training?-no char'].values=="Y" # Gets the indices of the training set
testing_set = pd.read_excel(file_name, sheet_name="Names",header=0).loc[:,'Testing?-no char'].values=="Y" # Gets the indices of the testing set
...
filename=path_prev+'/Training_Results/CompositionResults_noncharmodel.xlsx'
